...
namespace AdvancedWorld
{
    public class SWATHeli : Emergency
    {
        ...
        public override bool IsCreatedIn(Vector3 position, List<string> models)
        {
            ...
            foreach (Ped p in members)
            {
                if (p.Equals(spawnedVehicle.Driver)) Function.Call(Hash.TASK_HELI_MISSION, p, spawnedVehicle, 0, target, 0.0f, 0.0f, 0.0f, 4, 50.0f, 1.0f, (target.Position - spawnedVehicle.Position).ToHeading(), 15, 15, -1.0f, 1);
